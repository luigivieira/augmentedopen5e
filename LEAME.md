...
2. **Validación del slug** — El edge consulta un índice KV con todos los slugs de hechizos válidos de Open5e. Si el slug no está en la lista, devuelve `404 Not Found` de inmediato. En la primera solicitud (cold start), este índice se obtiene de forma síncrona desde Open5e y luego se almacena en caché — todas las solicitudes posteriores usan la copia local sin llamadas externas.

3. **Consulta al caché** — El edge verifica KV Storage para el par `slug + locale`.
   - **Cache hit** → `200 OK` con el hechizo traducido. No se realiza ninguna llamada externa.

4. **Cache miss** — El edge verifica si el contenido base en inglés (`en-us`) ya está en caché.
...
5. **Estado pendiente** — Si ya hay un job en segundo plano corriendo para ese `slug + locale`, la solicitud devuelve `202 Accepted` sin disparar un job duplicado.

6. **Polling del cliente** — En `202`, el cuerpo de la respuesta contiene un campo `progress` que indica el estado del job en segundo plano:
...
7. **Respuestas de error** — `400` para entrada inválida; `404` para slugs desconocidos; `500` para errores inesperados como timeouts en el edge, que no deberían ocurrir en condiciones normales.

### Estructura de Almacenamiento en Caché

Todos los datos en caché se almacenan en **Azion KV Storage** (Edge KV) en un único bucket por entorno:

| Entorno | Nombre del Bucket |
|---------|-------------------|
| Staging | `augmentedopen5e-staging-bucket` |
| Producción | `augmentedopen5e-prod-bucket` |

Dentro de cada bucket, se utilizan los siguientes patrones de clave:

| Patrón de Clave | Esquema del Valor | Descripción |
|-----------------|-------------------|-------------|
| `spell_{slug}_{locale}` | `SpellData & { locale: string }` | Datos del hechizo traducido para un slug y locale específicos |
| `catalog_{locale}` | `string[]` | Lista de todos los slugs ya almacenados en caché para un locale dado |
| `locales_index` | `string[]` | Lista de todos los locales que tienen al menos un hechizo en caché |
| `pending_{slug}_{locale}` | `{ pending: boolean }` | Marcador de traducción en curso para evitar trabajos duplicados en segundo plano |
| `valid_slugs_index` | `string[]` | Lista completa de slugs válidos de Open5e, obtenida una vez en el cold start |

Todos los valores se serializan como JSON y se almacenan como bytes codificados en UTF-8. Localmente, el emulador persiste los datos de KV en `.edge/storage/<nombre-del-bucket>/` en la raíz del proyecto.
...
Para generar un informe de cobertura (guardado en `coverage/` y mostrado como resumen en la terminal):

```bash
pnpm coverage
```

Para abrir la UI interactiva de cobertura en el navegador:
...
Dos workflows están configurados en `.github/workflows/`:

| Workflow | Archivo | Disparador |
|----------|---------|------------|
| **CI** | `ci.yml` | Cada push (todas las ramas) y PRs con destino a `main` |
| **Deploy** | `deploy.yml` | Tras el éxito del CI en `main` |

El workflow de **CI** ejecuta las pruebas unitarias (`pnpm test`) y luego verifica que la cobertura alcance el umbral mínimo del **90%** en statements, branches, functions y lines (`pnpm coverage`). Si alguno de los dos pasos falla, el CI falla y el despliegue queda bloqueado.

El workflow de **Deploy** utiliza `workflow_run` para dispararse únicamente después de que el workflow de CI concluya **exitosamente** en `main`. Esto garantiza que ningún código llega a producción sin pasar las pruebas y la verificación de cobertura. Los pushes directos a `main` sin que el CI haya pasado no activarán el despliegue.
